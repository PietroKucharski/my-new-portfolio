  ...
  return (
    <div className="mb-12 md:mb-16">
      <Reveal className="flex items-center gap-4 mb-5">
        <span className="font-mono text-acid text-sm tracking-widest">{index}</span>
        <span className="h-px w-10 bg-line" />
        <span className="font-mono text-dim text-xs tracking-[0.3em] uppercase">{label}</span>
      </Reveal>
      <Reveal delay={80}>
      ...
      {kicker && (
        <Reveal delay={140}>
          <p className="mt-5 max-w-xl text-dim text-lg leading-relaxed">{kicker}</p>
        </Reveal>
      )}
    </div>
  );
